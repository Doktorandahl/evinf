...
`alpha_pl_floor` in [`evinf_control`](evinf_control.md)), and, for a
bootstrapped model, the bootstrap replicate counts (`NA` otherwise):
`n_bootstraps` (usable), `n_failed_bootstraps`,
...
#> # ℹ 6 more variables: n_em_steps <int>, min_alpha_pl <dbl>, n_bootstraps <int>,
