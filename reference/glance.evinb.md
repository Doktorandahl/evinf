...
above C_EV, the smallest fitted Pareto shape (`min_alpha_pl`; see
`alpha_pl_floor` in [`evinf_control`](evinf_control.md)), `sum_weights`
(round9 D.2: the sum of `weights =`, equal to `nobs` for an unweighted
fit – this, not `nobs`, is what `aic`/`bic` and the model's degrees of
freedom are computed from), and, for a bootstrapped model, the bootstrap
replicate counts (`NA` otherwise): `n_bootstraps` (usable),
`n_failed_bootstraps`, `n_degenerate_bootstraps` — these three partition
the number of replicates requested (see
[`evinf_control`](evinf_control.md) for "degenerate") — and
`n_c_on_boundary`, the number of replicates whose C_EV landed on a
candidate-grid endpoint (informational, not counted as degenerate).
...
glance(model)
#> # A tibble: 1 × 17
#>    nobs sum_weights  npar alpha parameter   aic   bic logLik converged
#>   <int>       <dbl> <int> <dbl>     <dbl> <dbl> <dbl>  <dbl> <lgl>    
#> 1   100         100    14  10.0       184  550.  587.  -261. TRUE     
#> # ℹ 8 more variables: c_converged <lgl>, n_above_c <int>, n_em_steps <int>,
